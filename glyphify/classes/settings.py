...
from glyphify.utils.validate import are_keys_present, add_missing_values
...
DEFAULT_SETTINGS: dict[str, Any] = {
    "General": {"Interface": {"Theme": "Light"}, "Language": "en_US"}
}
...
class TomlSettings:
    ...
    def load_settings(self, default_settings: Optional[dict[str, Any]] = None) -> None:
        if self.logger:
        ...
        if os.path.exists(self.path):
            try:
                ...
                if not are_keys_present(loaded_toml_data, default_settings):
                    ...
                    loaded_toml_data = add_missing_values(
                        loaded_toml_data, default_settings
                    )
                ...
                if self.logger:
                    self.logger.error(
                        f'Failed to load settings from "{self.path}": {e}'
                    )
    ...
    def save_settings(self) -> None:
        ...
        if self.logger:
            self.logger.success(
                f'Settings were successfully saved to a file "{self.path}"!'
            )
